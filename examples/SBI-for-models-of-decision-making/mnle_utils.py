...
class MNLE(nn.Module):
    ...
    def log_prob(
        ...
        """
        theta = torch.unique(theta_repeated, sorted=False, dim=0)
        x = torch.unique(x_repeated, sorted=False, dim=0)
        num_parameters = theta.shape[0]
        assert x_repeated.ndim > 1
        assert (
            x_repeated.shape[1] == 2
        ), "MNLE assumes x to have two columns: [rts; choices]"
        rts_repeated = x_repeated[:, 0:1]
        choices_repeated = x_repeated[:, 1:2]
        rts = x[:, 0:1]
        choices = x[:, 1:2]
        ...
        lp_combined = (lp_choices + lp_rts).reshape(-1, num_parameters)
